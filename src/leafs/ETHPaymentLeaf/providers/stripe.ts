...
export class StripeProvider implements PaymentProvider {
    ...
    private static readonly DEFAULT_API_VERSION = '2024-12-18.acacia'

    async initialize(config: StripeConfig): Promise<void> {
        this.config = config
        this.stripe = new Stripe(config.secret_key, {
            apiVersion: (config.api_version as any) || StripeProvider.DEFAULT_API_VERSION
        })
    }
    ...
    async cancelSubscription(subscriptionId: string, immediate = false): Promise<SubscriptionResult> {
        if (immediate) {
            const subscription = await (this.stripe.subscriptions as any).cancel(subscriptionId)
            return this.mapSubscription(subscription)
        }
    ...
    async getRefund(refundId: string): Promise<RefundResult | null> {
        try {
            ...
            return {
                id: refund.id,
                provider: this.name,
                payment_id: typeof refund.payment_intent === 'string' ? refund.payment_intent : refund.payment_intent?.id || '',
                status: refund.status === 'succeeded' ? 'succeeded' : 'pending',
                amount: {
                    amount: refund.amount,
                    currency: refund.currency
    ...
    async initMobilePayment(options: MobilePaymentInitOptions): Promise<MobilePaymentInitResult> {
        ...
        const ephemeralKey = await this.stripe.ephemeralKeys.create({ customer: options.customer_id }, { apiVersion: this.config.api_version || StripeProvider.DEFAULT_API_VERSION })
     ...
     * Returns client_secret and ephemeral_key for the Stripe mobile SDK
     */
    async initMobileSetup(customerId: string): Promise<{
        provider: string
        customer_id: string
        ...
        const ephemeralKey = await this.stripe.ephemeralKeys.create({ customer: customerId }, { apiVersion: this.config.api_version || StripeProvider.DEFAULT_API_VERSION })
