...
export default class ETHUserLeafAuthController {
    ...
    public async setUserPassword(req: Request & { form: CreatePasswordFormType; user: UserLeafBase }, res: Response): Promise<any> {
        try {
    ...
    public async userUpdatePassword(req: Request & { form: UpdatePasswordFormType; user: UserLeafBase }, res: Response): Promise<any> {
        try {
            const { User } = getModels()
            const user = await User.unscoped().findByPk(req.user.id)
    ...
    public async confirmPasswordReset(req: Request & { form: PasswordResetConfirmFormType }, res: Response): Promise<any> {
        try {
            ...
            if (!user || !user.isPasswordResetTokenValid(req.form.token)) {
                ...
                if (user) {
                    await user.increment('password_reset_attempts')
                }
                res.error({
                    status: 400,
                    errors: ['api.password.reset_token_invalid'],
