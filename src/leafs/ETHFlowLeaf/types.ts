...
// ─── What a step exposes downstream ───────────────────────────────────────
...
export type ConfigSpec =
    | FieldDefinition[]
    | ((config: Record<string, any>) => FieldDefinition[])
...
export interface RunContext {
    ...
    actor: { type: 'system' | 'user' | 'guest'; id?: number }
    state: MachineState
    /** Innermost loop, when inside one. `{{item.*}}` / `{{loop.*}}`. */
    iterator?: { item: any; index: number; total: number }
...
export interface StepLog {
    ...
    output?: Record<string, any>
    branch?: string
    messages?: { text: string; meta?: Record<string, any> }[]
    error?: string
}
