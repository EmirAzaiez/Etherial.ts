...
export function nodeById(graph: FlowGraph, id: string): GraphNode | undefined {
    return graph.nodes.find((node) => node.id === id)
}

export function entryNode(graph: FlowGraph): GraphNode | undefined {
    return graph.nodes.find((node) => node.kind === 'trigger')
}

export function outgoing(graph: FlowGraph, nodeId: string): GraphEdge[] {
 ...
 */
export function successors(graph: FlowGraph, nodeId: string, branch?: string): string[] {
    const edges = outgoing(graph, nodeId)
    const selected = branch === undefined ? edges : edges.filter((edge) => (edge.branch ?? 'next') === branch)
    return selected.map((edge) => edge.target)
}

/** Every node reachable from a set of starts, following edges forward. */
export function reachableFrom(graph: FlowGraph, starts: string[]): Set<string> {
    ...
    while (queue.length > 0) {
        ...
        seen.add(id)
        for (const edge of outgoing(graph, id)) queue.push(edge.target)
    }
 ...
 */
export function loopTailOnly(graph: FlowGraph, bodyStarts: string[], doneStarts: string[]): string[] {
 ...
 */
export function findCycle(graph: FlowGraph): string[] | null {
