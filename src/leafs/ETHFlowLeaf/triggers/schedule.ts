...
import type { FieldDefinition, TriggerDefinition } from '../types.js'
...
function cadenceFields(defaultMode = 'daily'): FieldDefinition[] {
    return [
        {
            name: 'mode',
            type: 'select',
...
export const queryTrigger: TriggerDefinition = {
    id: 'schedule.query',
    label: 'When a record reaches a date',
    description:
        'Checks regularly for records whose date falls inside a window — "starts within 24 hours", "was created more than 3 days ago" — and runs once per record.',
    icon: 'CalendarClock',
    kind: 'query',
    group: 'Schedule',
    ...
    config: (config: Record<string, any>) => [
        ...
        },
        {
            ...
            required: true,
            options: dateFieldOptions(config?.collection),
        ...
        },
        {
            ...
            min: 0,
            helpText:
                'Leave at 0 for "within". Set it to narrow the window — 23 and 24 hours together mean "roughly a day before".',
        },
        {
            name: 'filters',
            type: 'json',
            ...
            options: [
                ...filterableFields(config?.collection).map((field) => ({
...
export function registerScheduleTriggers(): void {
...
function isDue(flow: Flow, now: Date, timezone?: string): boolean {
    const expression = cronFromConfig(flow.trigger_config ?? {})
    if (!expression) return false
    if (!matchesCron(expression, zonedMinute(now, timezone))) return false
    ...
    if (flow.last_tick_at) {
        ...
        if (Math.floor(previous.getTime() / 60000) === Math.floor(now.getTime() / 60000)) return false
    }
 ...
 */
async function claimTick(flow: Flow, now: Date): Promise<boolean> {
    const [claimed] = await Flow.update({ last_tick_at: now } as any, {
        where: { id: flow.id, last_tick_at: (flow.last_tick_at ?? null) as any },
    })
...
async function tickQueryFlow(flow: Flow, now: Date): Promise<number> {
    const config = flow.trigger_config ?? {}
    const collectionName = config.collection
    const dateField = config.date_field
    if (!collectionName || !dateField) return 0
    ...
    for (const record of records) {
        try {
            const outcome = await runFlow(flow, { triggerId: flow.trigger_id, record })
            if (outcome.run) started += 1
        } catch (error) {
 ...
 */
export async function runScheduleTick(now = new Date()): Promise<{ flows: number; runs: number }> {
    ...
    const flows = await Flow.findAll({
        where: {
            ...
            trigger_id: { [Op.in]: ['schedule.cron', 'schedule.query'] } as any,
        },
        order: [['id', 'ASC']],
    })
    ...
    for (const flow of flows) {
        try {
            if (!isDue(flow, now, timezone)) continue
            if (!(await claimTick(flow, now))) continue
            touched += 1

            if (flow.trigger_id === 'schedule.query') {
                runs += await tickQueryFlow(flow, now)
                continue
            }

            const outcome = await runFlow(flow, { triggerId: flow.trigger_id })
            if (outcome.run) runs += 1
        } catch (error: any) {
