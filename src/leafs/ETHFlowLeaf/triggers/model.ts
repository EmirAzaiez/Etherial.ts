...
export function installModelHooks(): number {
    if (leafConfig().modelTriggers === false) return 0

    let hooked = 0

    for (const collection of listCollections()) {
        const model: any = collection.model
        if (!model || typeof model.addHook !== 'function') continue
        if (model[HOOKED]) continue
        model[HOOKED] = true

        const name = collection.name

        model.addHook('afterCreate', 'eth_flow_created', (instance: any) => {
            if (!hasListener(`model.${name}.created`)) return
        ...
        model.addHook('beforeUpdate', 'eth_flow_snapshot', (instance: any) => {
            if (!hasListener(`model.${name}.updated`)) return
            try {
                instance[PREVIOUS] = { ...instance.previous() }
            } catch {
                instance[PREVIOUS] = {}
            }
        })

        model.addHook('afterUpdate', 'eth_flow_updated', (instance: any, options: any) => {
            if (!hasListener(`model.${name}.updated`)) return
            const changed: string[] = options?.fields ?? instance.changed() ?? []
            fireAndForget({
        ...
        model.addHook('afterDestroy', 'eth_flow_deleted', (instance: any) => {
            if (!hasListener(`model.${name}.deleted`)) return
        ...
        model.addHook('beforeBulkUpdate', 'eth_flow_bulk_update', (options: any) => {
            if (options.individualHooks) return
            if (!collectionHasListener(name)) return
            options.individualHooks = true
        })

        model.addHook('beforeBulkDestroy', 'eth_flow_bulk_destroy', (options: any) => {
            if (options.individualHooks) return
            if (!collectionHasListener(name)) return
            options.individualHooks = true
        })
