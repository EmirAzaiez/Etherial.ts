...
function collectionField(label = 'In'): FieldDefinition {
    return {
        name: 'collection',
        type: 'select',
        label,
...
function filtersField(collectionName: string | undefined, label = 'Where'): FieldDefinition {
    return {
        name: 'filters',
        type: 'json',
        widget: 'flow-filters',
        ...
        defaultValue: [],
        options: [
            ...filterableFields(collectionName ?? '').map((field) => ({
 ...
 * `{{record.starts_at}}`.
 */
function valuesField(collectionName: string | undefined): FieldDefinition {
...
function orderFields(collectionName: string | undefined): FieldDefinition[] {
    return [
        {
            name: 'order_by',
            type: 'select',
            ...
            options: filterableFields(collectionName ?? ''),
            showIf: { field: 'collection', operator: 'truthy' },
        },
        {
            name: 'order_dir',
...
function toValues(rows: any): Record<string, any> {
...
function requireModel(collectionName: string): any {
    const model = getModel(collectionName)
    if (!model) throw new Error(`Unknown collection "${collectionName}"`)
    return model
}

export const findNode: NodeDefinition = {
    id: 'data.find',
    label: 'Find records',
    ...
    config: (config) => [
        collectionField(),
        filtersField(config?.collection),
        ...orderFields(config?.collection),
    ...
    output: (config) => [
        {
            ...
            fields: config?.collection ? collectionOutputFields(config.collection) : undefined,
        },
    ...
    execute: async ({ config, log }) => {
        ...
        const records = await Promise.all(rows.map((row: any) => hydrateRecord(config.collection, row)))
...
export const findOneNode: NodeDefinition = {
    id: 'data.find_one',
    ...
    execute: async ({ config, log }) => {
        ...
        const record = await hydrateRecord(config.collection, row)
        log(`Found #${record.id}`)
        return { branch: 'found', output: { record, found: true } }
    },
}

export const createNode: NodeDefinition = {
    id: 'data.create',
    label: 'Create a record',
    description: 'Adds a row.',
    icon: 'FilePlus',
    category: 'data',
    config: (config) => [collectionField(), valuesField(config?.collection)],
    output: (config) => [
        {
            key: 'record',
    ...
    execute: async ({ config, log }) => {
        ...
        const record = await hydrateRecord(config.collection, created)
...
export const updateNode: NodeDefinition = {
    id: 'data.update',
    ...
    config: (config) => [
        ...
        { ...filtersField(config?.collection), showIf: { field: 'target', operator: 'eq', value: 'filters' } },
        valuesField(config?.collection),
        {
            name: 'limit',
            type: 'number',
            label: 'At most',
    ...
    execute: async ({ config, log }) => {
        ...
        if (config.target === 'filters') {
            ...
            for (const row of rows) await row.update(values)
            log(`${rows.length} record${rows.length === 1 ? '' : 's'} updated`)
            return { output: { updated: rows.length } }
        }

        const row = await model.findByPk(config.record_id)
        if (!row) {
        ...
        return { output: { updated: 1, record: await hydrateRecord(config.collection, row) } }
    },
}

export const deleteNode: NodeDefinition = {
    id: 'data.delete',
...
export const countNode: NodeDefinition = {
    ...
    icon: 'Hash',
    category: 'data',
    config: (config) => [collectionField(), filtersField(config?.collection)],
    output: [{ key: 'count', label: 'Count', type: 'number' }],
    execute: async ({ config, log }) => {
        const model = requireModel(config.collection)
        const count = await model.count({ where: toWhere(config.filters, 'all') })
        log(`${count}`)
        return { output: { count } }
    },
}
