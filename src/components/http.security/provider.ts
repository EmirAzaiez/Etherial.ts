 ...
 * ```
 */
function ShouldUseLimiter(config: RateLimitConfig): MethodDecorator {
 ...
 * ```
 */
function ShouldUseStrictLimiter(): MethodDecorator {
 ...
 * ```
 */
function ShouldUseRelaxedLimiter(): MethodDecorator {
    return Middleware(etherial['http_security'].createRateLimiter({
        windowMs: 60000,
        max: 100
    }))
 ...
 * ```
 */
function ShouldUseLimiterPerUser(config: Omit<RateLimitConfig, 'keyGenerator'>): MethodDecorator {
    return Middleware(etherial['http_security'].createRateLimiter({
        ...config,
        keyGenerator: (req) => {
 ...
 * ```
 */
function ShouldBlockIPs(ips: string[]): MethodDecorator {
 ...
 * ```
 */
function ShouldAllowOnlyIPs(ips: string[]): MethodDecorator {
 ...
 * ```
 */
function ShouldFilterIPs(config: IPFilterConfig): MethodDecorator {
 ...
 * ```
 */
function ShouldUseStrictBruteForce(): MethodDecorator {
 ...
 * ```
 */
function ShouldLimitSize(maxSize: number): MethodDecorator {
 ...
 */
function ShouldLimitTo1KB(): MethodDecorator {
 ...
 */
function ShouldLimitTo100KB(): MethodDecorator {
 ...
 */
function ShouldLimitTo1MB(): MethodDecorator {
 ...
 */
function ShouldLimitTo10MB(): MethodDecorator {
 ...
 * ```
 */
function ShouldSecureAuthRoute(): MethodDecorator {
    return Middleware([
 ...
 * ```
 */
function ShouldSecureAPIRoute(): MethodDecorator {
    return Middleware([
